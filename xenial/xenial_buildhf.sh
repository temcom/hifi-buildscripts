...
# make files (a routine occurence) and certain packages being at certain level in Artful.  It's likely
# Artful will remain compatible through its lifespan (Summer 2018) - unless HiFi moves to a new
# bleeding edge.
#
#!/bin/bash
# This looks for most recent tag and builds or warns and exits if already built.
set -e
export QT_CMAKE_PREFIX_PATH=/opt/qt59/lib/cmake
...
chrpath -r $HFBIN:/opt/qt59/lib assignment-client
...
patchelf --set-rpath /opt/qt59/lib libquazip5.so.1.0.0
...
strip -s interface
chrpath -r $HFBIN:/opt/qt59/lib interface
mv interface $HFBIN
cd $HFSRC/build/interface/plugins
strip libhifiCodec.so
mv libhifiCodec.so $HFBIN/plugins
strip libhifiSdl2.so
chrpath -r $HFBIN:/opt/qt59/lib libhifiSdl2.so
mv libhifiSdl2.so $HFBIN/plugins
strip libhifiSixense.so
chrpath -r $HFBIN:/opt/qt59/lib libhifiSixense.so
